...
const OrdersPage: React.FC<OrdersPageProps> = ({ 
    ...
    const handleInvoiceSent = async (order: Order) => {
        ...
        try {
            ...
            // 3. Sync search results if they are currently being viewed
            if (searchResults) {
                setSearchResults(prev => prev ? prev.map(o => o.id === order.id ? { ...o, isInvoicePrinted: true } : o) : null);
            }
            
            showToast('تم تأكيد إرسال الفاتورة وتحديث الحالة بصرياً', 'success');
        } catch (e: any) {
            showToast(getErrorMessage(e), 'error');
        }
    };
