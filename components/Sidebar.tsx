...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  // 2. Sort based on settings.viewOrder
  ...
  }, [permissionFilteredItems, settings?.viewOrder]);
  ...
  const { primaryItems, menuItems } = useMemo(() => {
      const preferredViews = settings?.mobileDockViews || ['dashboard', 'orders', 'delivery', 'clients'];
  ...
  }, [sortedItems, settings?.mobileDockViews]);
