...
const SecurityModal: React.FC<{
    ...
    return (
         <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="bg-content-light dark:bg-content-dark rounded-xl shadow-2xl p-6 w-full max-w-md max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
                    ...
                    {!isSelf && (
                        <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-xl border border-green-200 dark:border-green-800">
                            ...
                            <p className="text-xs text-gray-600 dark:text-gray-400 mb-3">
                                إذا كان المستخدم يواجه مشكلة "البريد غير مؤكد" عند الدخول، اضغط هنا لتفعيله فوراً.
                            </p>
                            <button 
...
const UserCard: React.FC<{ 
    ...
    return (
        <div className="bg-content-light dark:bg-content-dark rounded-xl shadow-lg p-5 flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border border-gray-100 dark:border-gray-800">
            <div>
            ...
            <div className="mt-4 flex flex-wrap justify-end items-center border-t border-gray-200 dark:border-gray-700 pt-3 gap-2">
                <button onClick={() => onSecurity(user)} className="p-2 bg-yellow-50 text-yellow-600 rounded-lg hover:bg-yellow-100 dark:bg-yellow-900/20 dark:text-yellow-400 dark:hover:bg-yellow-900/40 transition-colors" title="إدارة الأمان والتفعيل">
                    <Key size={18} />
                </button>
...
const UsersPage: React.FC<UsersPageProps> = ({ users, setUsers, logAction, globalActivityLog }) => {
    ...
    const handleSaveUser = async (userData: User, email?: string, password?: string) => {
        ...
        try {
            ...
            if (userData.id) {
                ...
                // Force email confirmation on update too, just in case they were stuck
                // Note: We swallow errors here because if the SQL function is missing, we still want to update profile
                try {
                ...
                if (authData.user) {
                    ...
                    // --- 3. Auto-Confirm Email (Fix for "Email not confirmed") ---
                    const { error: confirmError } = await supabase.rpc('admin_confirm_user_email', { target_user_id: newUserId });
                    
                    if (confirmError) {
                        // If function missing, user needs to update DB.
                        if (confirmError.code === '42883' || confirmError.code === 'PGRST202' || confirmError.message?.includes('function not found')) {
                             setIsDbSetupOpen(true);
                             throw new Error("تم إنشاء الحساب ولكن فشل تفعيله تلقائياً. يرجى تحديث قاعدة البيانات (الزر في الأعلى) لتمكين التفعيل التلقائي.");
                        }
    ...
    const handleConfirmDelete = async (password: string) => {
        ...
        // Try getting admin email from context, if not fall back to session
