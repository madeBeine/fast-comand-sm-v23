...
// Helper to ensure image source is valid (Handling old Base64 strings)
const resolveImageSrc = (src: string | undefined | null) => {
    if (!src) return '';
    if (src.startsWith('http') || src.startsWith('blob:') || src.startsWith('data:')) {
        return src;
    }
    // Fix for older images missing the data URI prefix
                                                                ...
                                                                <div className="flex gap-2">
                                                                    {pay.receiptImages.map((img, idx) => (
                                                                        <img 
                                                                            ...
                                                                            className="w-12 h-12 object-cover rounded-lg border cursor-pointer hover:scale-110 transition-transform bg-white" 
                ...
                >
                    ...
                    >
                        <img 
                            ...
                            className="max-w-[95%] max-h-[90vh] object-contain select-none pointer-events-auto bg-white rounded-lg shadow-2xl"
                            draggable={false}
                            onError={(e) => {
                                (e.target as HTMLImageElement).style.display = 'none';
                                alert("تعذر تحميل الصورة. قد يكون الملف تالفاً.");
                            }}
                        />
                    </div>
                </div>
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {images.map((src, i) => (
                    <div key={i} className="group relative aspect-square bg-white dark:bg-gray-800 rounded-xl overflow-hidden border dark:border-gray-700 cursor-pointer shadow-sm" onClick={() => onImageClick(src)}>
                        <img 
