...
import { X, User, Store as StoreIcon, Calendar, DollarSign, Truck, Package, FileText, Image as ImageIcon, Link as LinkIcon, Loader2, ZoomIn, Plus, Minus, RotateCcw } from 'lucide-react';
...
const OrderDetailsModal: React.FC<OrderDetailsModalProps> = ({ isOpen, onClose, order, client, store, shippingCompanies = [] }) => {
    ...
    useEffect(() => {
        if (isOpen && order) {
            setFullOrder(order);
            // Fetch images if needed or if they are lazy loaded in list
            if (supabase) {
                const fetchDetails = async () => {
                    ...
                    if (data) {
                        setFullOrder({
                            ...
                            trackingImages: data.tracking_images || [],
            ...
            setPreviewImage(null); // Reset lightbox on close
        }
    }, [isOpen, order]);
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl border dark:border-gray-700 shadow-sm">
                                        ...
                                        <div className="space-y-3 text-sm">
                                            ...
                                            <div className="flex justify-between">
                                                <span className="text-gray-500">الرقم العالمي:</span>
                                                <span className="font-mono font-bold select-all">{fullOrder.globalOrderId || '---'}</span>
                                            </div>
                                            <div className="flex justify-between">
                        ...
                        {activeTab === 'images' && (
                            <div className="space-y-8">
                                {isLoading ? <Loader2 className="mx-auto animate-spin"/> : (
                                    <>
                                        <ImageSection title="صور المنتج (Product)" images={fullOrder.productImages} onImageClick={setPreviewImage} />
                                        <ImageSection title="مرفقات الطلب (Attachments)" images={fullOrder.orderImages} onImageClick={setPreviewImage} />
                                        <ImageSection title="صور التتبع والشحن (Tracking)" images={fullOrder.trackingImages} onImageClick={setPreviewImage} />
                                        <ImageSection title="صور الوصول للمحطة (Hub)" images={fullOrder.hubArrivalImages} onImageClick={setPreviewImage} />
                                        <ImageSection title="صور الوصول للمكتب (Weighing)" images={fullOrder.weighingImages} onImageClick={setPreviewImage} />
                                        <ImageSection title="إيصالات الدفع (Receipts)" images={allReceipts} onImageClick={setPreviewImage} />
                                    </>
                                )}
                            </div>
                        )}
                        ...
                        {activeTab === 'history' && (
                            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl border dark:border-gray-700">
                                <ul className="space-y-6 border-l-2 border-gray-200 dark:border-gray-700 ml-3 pl-6 relative">
                                    ...
                                    {(!fullOrder.history || fullOrder.history.length === 0) && (
        ...
        <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {images.map((src, i) => (
                    <div 
                        key={i} 
                        className="group relative aspect-square bg-gray-100 dark:bg-gray-800 rounded-xl overflow-hidden border dark:border-gray-700 cursor-pointer" 
