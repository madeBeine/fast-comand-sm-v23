...
import { 
    ...
    Calendar, MapPin, Scale, Loader2, Container
} from 'lucide-react';
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async (event) => {
            ...
            img.onload = () => {
                ...
                // FIX: Fill white background to prevent black transparent areas
...
const ShipmentDetailsModal: React.FC<{ 
    ...
    const totalBoxes = shipment.numberOfBoxes || 0;
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b dark:border-gray-700 flex justify-between items-start bg-gray-50/50 dark:bg-gray-900/50">
                    <div>
                        ...
                        <h2 className="text-3xl font-black text-gray-800 dark:text-white font-mono">{shipment.shipmentNumber}</h2>
                    </div>
                ...
                <div className="flex-grow overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-2xl border border-purple-100 dark:border-purple-800">
                            <span className="text-xs text-purple-600 dark:text-purple-300 font-bold block mb-1">طريقة النقل</span>
                            <div className="flex items-center gap-2 font-bold text-gray-800 dark:text-white">
                                {shipment.transportMode === 'air' ? <Plane size={18}/> : shipment.transportMode === 'sea' ? <Ship size={18}/> : <Truck size={18}/>}
                                {shipment.transportMode === 'air' ? 'جوي' : shipment.transportMode === 'sea' ? 'بحري' : 'بري'}
                            </div>
                        </div>
                        <div className="p-4 bg-gray-50 dark:bg-gray-700/30 rounded-2xl border dark:border-gray-700">
                            ...
                            <div className="flex items-center gap-2 font-mono font-bold">{shipment.departureDate}</div>
                        </div>
                        <div className="p-4 bg-gray-50 dark:bg-gray-700/30 rounded-2xl border dark:border-gray-700">
                            <span className="text-xs text-gray-500 font-bold block mb-1">الوصول المتوقع</span>
                            <div className="flex items-center gap-2 font-mono font-bold">{shipment.expectedArrivalDate}</div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4 p-4 bg-gray-100 dark:bg-gray-900 rounded-2xl">
                        ...
                        <div className="flex-1 text-center">
                            ...
                            <p className="text-xl font-black text-orange-600">{shipment.totalWeight || 0} <span className="text-xs">KG</span></p>
                        </div>
                    </div>
                    ...
                    {shipment.receiptImage && (
                        <div>
                            ...
                            <div className="rounded-xl overflow-hidden border dark:border-gray-700">
                                <img src={shipment.receiptImage} className="w-full object-contain max-h-60 bg-gray-50" alt="Receipt"/>
                            </div>
                        </div>
                    )}
                </div>
    ...
    useEffect(() => {
        if (isOpen) {
            if (shipment) {
                ...
                setFormData({
                    shipmentNumber: `SH-${Math.floor(Math.random() * 10000)}`, // Simple ID gen
        ...
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-[60] p-4 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-end gap-3 rounded-b-3xl">
                    ...
                    <button onClick={handleSave} disabled={isSaving || isProcessing} className="px-8 py-2.5 bg-primary hover:bg-primary-dark text-white rounded-xl font-bold shadow-lg flex items-center gap-2 disabled:opacity-50">
...
const ShipmentStatusUpdateModal: React.FC<{
    ...
    useEffect(() => {
        if (isOpen && shipment) {
            ...
            setBoxes(JSON.parse(JSON.stringify(shipment.boxes || []))); // Deep copy
        }
    }, [isOpen, shipment]);
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            // Auto-update status if all boxes arrived
        ...
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-[60] p-4 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="grid grid-cols-4 gap-2 max-h-48 overflow-y-auto custom-scrollbar p-1">
                                {boxes.map(box => (
                                    <button 
                                        ...
                                        className={`p-2 rounded-lg border text-xs font-bold transition-all ${box.status === 'arrived' ? 'bg-green-500 text-white border-green-600' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                                    >
                                        #{box.boxNumber}
                                    </button>
                                ))}
                ...
                <div className="p-6 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-end gap-3">
                    ...
                    <button onClick={handleSave} disabled={isSaving} className="px-8 py-2.5 bg-primary text-white rounded-xl font-bold shadow-lg flex items-center gap-2">
    ...
    const handleSaveShipment = async (payload: Partial<Shipment>) => {
        ...
        try {
            const dbPayload: any = {
                ...
                shipping_type: ShippingType.NORMAL, // Default, implies cargo type usually
    ...
    return (
        <div className="space-y-6 pb-20">
            ...
            <div className="flex overflow-x-auto pb-2 gap-3 custom-scrollbar -mx-2 px-2 no-scrollbar">
                <FilterChip id="all" label="الكل" count={counts.all} icon={Truck} active={smartFilter === 'all'} onClick={() => setSmartFilter('all')} colorClass="bg-gray-100 text-gray-600"/>
                <FilterChip id="transit" label="في الطريق" count={counts.transit} icon={Zap} active={smartFilter === 'transit'} onClick={() => setSmartFilter('transit')} colorClass="bg-blue-100 text-blue-600"/>
                <FilterChip id="partial" label="وصول جزئي" count={counts.partial} icon={AlertOctagon} active={smartFilter === 'partial'} onClick={() => setSmartFilter('partial')} colorClass="bg-orange-100 text-orange-600"/>
                <FilterChip id="completed" label="واصلة" count={counts.completed} icon={CheckCircle} active={smartFilter === 'completed'} onClick={() => setSmartFilter('completed')} colorClass="bg-green-100 text-green-600"/>
            </div>
