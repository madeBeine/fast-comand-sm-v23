...
const InvoiceTemplate: React.FC<{
    ...
    const t = {
        ar: {
            title: 'فاتورة', billTo: 'بيانات العميل', invoiceDetails: 'تفاصيل الفاتورة',
            store: 'المتجر', description: 'البيان', price: 'السعر', commission: 'العمولة',
            ...
            currency: 'أوقية', qty: 'العدد', amount: 'المجموع (MRU)', productVal: 'قيمة المنتجات', fees: 'رسوم المكتب',
            generated: 'تم التوليد بواسطة', date: 'التاريخ', expected: 'توقع الوصول',
            originalPrice: 'السعر الأصلي', globalId: 'رقم الطلب العالمي'
        },
        en: {
            title: 'INVOICE', billTo: 'Bill To', invoiceDetails: 'Invoice Details',
        ...
        fr: {
            title: 'FACTURE', billTo: 'Facturer à', invoiceDetails: 'Détails',
            ...
            currency: 'MRU', qty: 'Qté', amount: 'Montant (MRU)', productVal: 'Valeur Produit', fees: 'Frais Bureau',
            generated: 'Généré par', date: 'Date', expected: 'Arrivée Prévue',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                        ...
                        <div>
                            ...
                            <p style={{ margin: '4px 0 0', opacity: 0.7, fontSize: '12px' }}>Fast Comand SM Logistics</p>
                        </div>
                    </div>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td style={{ ...tdStyle, textAlign: 'center', fontWeight: 'bold', fontFamily: 'monospace' }}>
                                {Number(order.price || 0).toLocaleString()} {order.currency}
                            </td>
...
const InvoiceModal: React.FC<InvoiceModalProps> = ({ isOpen, onClose, order, client, store, companyInfo, onSuccess }) => {
        ...
        try {
            ...
            if (action === 'print') {
                ...
                if (printWindow) {
                    ...
                    // CRITICAL: Mark as success IMMEDIATELY upon successful generation
                ...
                pdf.save(`Invoice-${order.localOrderId}.pdf`);
                ...
                // CRITICAL: Mark as success IMMEDIATELY upon successful generation
                // even before the user triggers the OS share sheet.
                if (onSuccess) onSuccess(); 
            }
        ...
        try {
            if (navigator.share && navigator.canShare && navigator.canShare({ files: [preparedFile] })) {
                await navigator.share({
                    ...
                    text: `فاتورة الطلب #${order.localOrderId} - ${companyInfo.name}`,
                });
            } else {
    ...
    if (!selectedLang) {
        return (
            <div className="fixed inset-0 bg-slate-900/95 z-[120] flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
                <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 w-full max-w-sm text-center animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                    ...
                    <h3 className="text-2xl font-black text-slate-800 dark:text-white mb-2">لغة الفاتورة</h3>
                    <p className="text-slate-500 dark:text-slate-400 mb-8 text-sm">اختر لغة العرض والمشاركة المفضلة للفاتورة</p>
                    ...
                    <button onClick={onClose} className="mt-8 text-slate-400 hover:text-slate-600 font-bold transition-colors">إلغاء</button>
                </div>
            </div>
        );
    }
            ...
            {genStatus !== 'idle' && (
                <div className="fixed inset-0 z-[10000] bg-white/95 dark:bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300">
                    ...
                    <h3 className="text-2xl font-black text-slate-800 dark:text-white mb-3">
                        {genStatus === 'generating' ? 'جاري تجهيز الفاتورة' : genStatus === 'ready_to_share' ? 'الفاتورة جاهزة للإرسال' : 'حدث خطأ ما'}
                    </h3>
                    <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto text-sm mb-10 leading-relaxed">
                        {genStatus === 'generating' 
                            ? 'نقوم الآن بتحويل الفاتورة إلى ملف PDF عالي الجودة متوافق مع كافة الأجهزة...' 
                            : genStatus === 'ready_to_share' 
                            ? 'تم إنشاء الملف بنجاح. اضغط على الزر أدناه لمشاركته عبر واتساب أو أي تطبيق آخر.'
                            : 'لم نتمكن من معالجة الفاتورة حالياً، يرجى المحاولة لاحقاً.'}
                    </p>
            ...
            <div className="fixed inset-0 bg-slate-900 z-[120] flex flex-col backdrop-blur-sm overflow-hidden animate-in fade-in">
                <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-center z-[150] pointer-events-none">
                    <button onClick={() => setSelectedLang(null)} className="p-3 bg-slate-800/80 hover:bg-slate-700 text-white rounded-full backdrop-blur-md pointer-events-auto transition-all flex items-center gap-2 px-5 shadow-lg border border-white/10 active:scale-95">
                        <ChevronLeft size={20}/> <span className="text-sm font-bold hidden sm:inline">تغيير اللغة</span>
                    </button>
                ...
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-white/10 backdrop-blur-xl border border-white/20 p-2 rounded-3xl flex gap-3 shadow-2xl z-[150] animate-in slide-in-from-bottom-10 duration-500">
                    ...
                    >
                        ...
                        <span className="text-[10px] font-black mt-1">مشاركة</span>
                    </button>
                    ...
                    >
                        ...
                        <span className="text-[10px] font-black mt-1">طباعة</span>
                    </button>
                    ...
                    >
                        ...
                        <span className="text-[10px] font-black mt-1">حفظ</span>
                    </button>
                </div>
            </div>
        </>
