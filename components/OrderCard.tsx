...
import { 
    ...
    Plane, Ship, Truck, CheckCircle2, Tag, Download, Loader2, Image as ImageIcon, ChevronRight, Package, Link, Wallet, FileText
} from 'lucide-react';
...
const OrderCard: React.FC<OrderCardProps> = ({ order, client, store, users = [], companyInfo, onEdit, onDelete, onCancel, onChangeStatus, onUpdatePayment, onHistory, onView, onSplit, onSendNotification, onInvoiceSent, onClientClick, searchTerm }) => {
    ...
    const handleGenerateLabel = async () => {
        ...
        try {
            ...
            setTimeout(() => setIsGeneratingLabel(false), 500);
        }
    };

    const handleLinksClick = () => {
        if (validLinks.length === 1) {
            window.open(validLinks[0], '_blank');
        ...
        <>
            ...
            {showLinksModal && (
                <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-[150] p-4 backdrop-blur-sm" onClick={() => setShowLinksModal(false)}>
                    <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-sm overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                        <div className="p-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center">
                            <h3 className="font-bold text-gray-800 dark:text-white flex items-center gap-2">
                                <Link size={18}/> روابط المنتج
                            </h3>
                            <button onClick={() => setShowLinksModal(false)} className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700">
                        ...
                        <div className="p-2 max-h-[60vh] overflow-y-auto custom-scrollbar">
                            {validLinks.map((link, idx) => (
                                <a 
                                    ...
                                    className="flex items-center gap-3 p-3 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-colors border-b last:border-0 border-gray-100 dark:border-gray-700"
                                >
                                    <div className="bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400 p-2 rounded-lg">
                                        <ExternalLink size={16}/>
                                    </div>
                                    <div className="overflow-hidden">
                                        <p className="text-xs font-bold text-gray-700 dark:text-gray-300 truncate w-full">رابط {idx + 1}</p>
                                        <p className="text-[10px] text-gray-400 truncate w-full">{link}</p>
                                    </div>
                                </a>
                            ))}
                        </div>
            ...
            `}>
                ...
                {/* --- INNOVATIVE FLOATING INVOICE BADGE --- */}
                {order.isInvoicePrinted && (
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 z-20">
                        <div className="bg-emerald-600/95 backdrop-blur-xl text-white text-[10px] px-7 py-1.5 rounded-b-[1.3rem] shadow-[0_4px_12px_rgba(16,185,129,0.3)] font-black flex items-center gap-2 border-x border-b border-white/20 animate-in slide-in-from-top-4 duration-700 ring-4 ring-emerald-500/10">
                            <div className="relative">
                                <CheckCircle2 size={12} strokeWidth={3} className="relative z-10" />
                                <div className="absolute inset-0 bg-white rounded-full blur-[2px] opacity-50 animate-ping"></div>
                            </div>
                            <span className="uppercase tracking-[0.1em] drop-shadow-sm">فاتورة مكتملة</span>
                        </div>
                    </div>
                )}
                ...
                <div className="mt-auto p-5 space-y-3">
                    {!isCompleted && order.status === OrderStatus.ORDERED ? (
                        <button 
                            ...
                            className={`w-full py-4 rounded-2xl font-black shadow-lg transition-all flex items-center justify-center gap-3 active:scale-95 group/btn ${order.isInvoicePrinted ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-600/30' : 'bg-primary hover:bg-primary-dark text-white shadow-primary/20'}`}
                        >
                            {order.isInvoicePrinted ? <FileText size={20}/> : <Share2 size={20} className="group-hover/btn:rotate-12 transition-transform"/>} 
                            {order.isInvoicePrinted ? 'إعادة مشاركة الفاتورة' : t('printInvoice')}
                        </button>
