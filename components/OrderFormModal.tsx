...
import { Save, X, Plus, Trash2, Loader2, Link, Ship, Plane, Truck, DollarSign, Image as ImageIcon, User, ShoppingBag, CheckCircle2, Calculator, Receipt, Upload, Search, Tag, Sparkles, Wand2, Clock } from 'lucide-react';
...
const OrderFormModal: React.FC<OrderFormModalProps> = ({ isOpen, onClose, onSave, order, clients, stores, currencies, commissionRate, settings, shippingCompanies = [], paymentMethods = [], orders = [], onClientSearch }) => {
    const { t } = useLanguage();
    ...
    /**
     * Fix: Added useCallback to define generateNextOrderId correctly.
     * useCallback must be imported from 'react'.
     */
    const generateNextOrderId = useCallback(() => {
        if (!settings?.orderIdPrefix) return '';
        
        // Find maximum numeric ID in the current loaded list
        let maxId = 0;
        const prefix = settings.orderIdPrefix.toUpperCase();
        
        orders.forEach(o => {
            const match = o.localOrderId.toUpperCase().match(new RegExp(`${prefix}(\\d+)`));
            if (match && match[1]) {
        ...
        let finalId = `${prefix}${nextNum}`;
        
        // Final safety check: if by any chance this ID exists in the local list, increment again
        while (orders.some(o => o.localOrderId.toUpperCase() === finalId.toUpperCase())) {
            maxId++;
            finalId = `${prefix}${maxId + 1}`;
        }
        
        return finalId;
    }, [orders, settings?.orderIdPrefix]);
    ...
    useEffect(() => {
        if (isOpen && order?.id) {
            ...
            // New Order: Generate Sequential ID
    ...
    }, [isOpen, order?.id, generateNextOrderId, settings?.defaultShippingType, settings?.defaultCurrency, settings?.defaultOriginCenter, currencies, paymentMethods]);
    ...
    const handleMagicFill = async () => {
        ...
        try {
            ...
            const response = await ai.models.generateContent({
                ...
                contents: `Analyze this shopping URL and extract the price, currency, and product name. URL: "${link}"`,
                config: { 
            ...
            console.error(e);
            showToast("فشل المساعد في استخراج البيانات. قد يكون الرابط محمياً أو غير مدعوم.", "error"); 
    ...
    const handleSubmit = async () => {
        ...
        // Prevent duplicate ID submission for new orders
        if (!order && orders.some(o => o.localOrderId.toUpperCase() === formData.localOrderId?.toUpperCase())) {
            newErrors.localOrderId = "رقم الطلب مسجل مسبقاً";
        }

        if (Object.keys(newErrors).length > 0) {
            setErrors(newErrors);
            return showToast(newErrors.localOrderId || "يرجى إكمال البيانات المطلوبة", "warning");
        }
