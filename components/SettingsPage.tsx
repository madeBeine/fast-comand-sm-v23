...
import { 
    ...
    Smartphone, FileText, Globe, Plane, Ship, Phone, Hash, Wallet, Coins, ArrowRightLeft, Calculator, RefreshCw, Database, Smartphone as SmartphoneIcon, ArrowUp, ArrowDown, Menu, Lock, FileSpreadsheet, Download, Sparkles, Clock
} from 'lucide-react';
...
const SettingsPage: React.FC<SettingsPageProps> = ({ 
    ...
    const [isActionVerifyOpen, setIsActionVerifyOpen] = useState(false);
    ...
    const initiateDataAction = (action: string) => {
        if (!isAdmin) return;
        setPendingDataAction(action);
        setIsActionVerifyOpen(true);
    };

    const handleActionConfirm = async (password: string) => {
        setIsActionVerifyOpen(false);
        
        switch (pendingDataAction) {
            case 'generate_demo': handleGenerateDemoData(); break;
            case 'export_clients': handleExportClients(); break;
            case 'export_orders': handleExportOrders(); break;
            case 'import_clients': clientsInputRef.current?.click(); break;
            case 'import_orders': ordersInputRef.current?.click(); break;
        }
    ...
    const renderModalContent = () => {
                ...
                <div className="space-y-6">
                    <div className="flex items-center gap-3 border-b pb-4 dark:border-gray-700">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">إدارة معلومات المتجر وإعدادات الشحن الافتراضية</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="md:col-span-2">
                            ...
                            <input value={editingItem.name || ''} onChange={e => setEditingItem({...editingItem, name: e.target.value})} className={inputClass} placeholder="مثال: Amazon, AliExpress..." />
                        </div>
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input value={editingItem.country || ''} onChange={e => setEditingItem({...editingItem, country: e.target.value})} className={`${inputClass} pl-10`} placeholder="الصين، الإمارات..." />
                            </div>
                        </div>
                        <div>
                            <label className={labelClass}>مدة التوصيل (أيام)</label>
                            <div className="relative">
                                <Clock size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"/>
                                <input type="number" value={editingItem.estimatedDeliveryDays || ''} onChange={e => setEditingItem({...editingItem, estimatedDeliveryDays: parseInt(e.target.value) || 0})} className={`${inputClass} pl-10`} placeholder="14" />
                            </div>
                        </div>
                        <div className="md:col-span-2">
                            <label className={labelClass}>رابط المتجر (اختياري)</label>
                            <input value={editingItem.website || ''} onChange={e => setEditingItem({...editingItem, website: e.target.value})} className={inputClass} placeholder="https://www.example.com" />
                        </div>
                    </div>

                    <div>
                        <label className={labelClass}>شعار المتجر</label>
                        <div className="flex items-center gap-5 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                            <div className="w-20 h-20 bg-white dark:bg-gray-700 rounded-xl flex items-center justify-center overflow-hidden border dark:border-gray-600 shadow-inner shrink-0">
                                {editingItem.logo ? <img src={editingItem.logo} className="w-full h-full object-contain"/> : <ShoppingBag className="text-gray-300" size={32}/>}
                            </div>
                            <div className="flex flex-col gap-2 flex-grow">
                                <label className="cursor-pointer bg-white dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 px-4 py-2 rounded-xl text-sm font-bold border dark:border-gray-600 transition-colors flex items-center justify-center gap-2">
                                ...
                                {editingItem.logo && (
                                    <button onClick={() => setEditingItem({...editingItem, logo: null})} className="text-xs text-red-500 font-bold hover:underline">إزالة الشعار</button>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="pt-4 flex justify-end gap-3 border-t dark:border-gray-700">
                        <button onClick={closeModal} className="px-6 py-2.5 rounded-xl text-gray-500 font-bold hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">إلغاء</button>
                        <button onClick={() => handleGenericSave('Stores', editingItem, setStores)} className="px-8 py-2.5 bg-primary hover:bg-primary-dark text-white rounded-xl font-bold shadow-lg shadow-primary/20 flex items-center gap-2">
                            {isSaving ? <Loader2 className="animate-spin" size={20}/> : <Save size={20}/>}
                            حفظ المتجر
                        </button>
                    </div>
        ...
        if (modalType === 'shipping_company') {
            return (
                <div className="space-y-5">
                    <h3 className="text-xl font-black border-b pb-3 dark:border-gray-700">شركة الشحن</h3>
                    <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className={labelClass}>بلد المنشأ</label>
                            <input value={editingItem.originCountry || ''} onChange={e => setEditingItem({...editingItem, originCountry: e.target.value})} className={inputClass} />
                        </div>
                        <div>
                            <label className={labelClass}>بلد الوصول</label>
                            <input value={editingItem.destinationCountry || ''} onChange={e => setEditingItem({...editingItem, destinationCountry: e.target.value})} className={inputClass} />
                        </div>
                    </div>
                    <div className="pt-4 flex justify-end gap-3">
                        <button onClick={closeModal} className="px-6 py-2 rounded-xl bg-gray-200 dark:bg-gray-700">إلغاء</button>
                        <button onClick={() => handleGenericSave('ShippingCompanies', editingItem, setShippingCompanies)} className="px-6 py-2 rounded-xl bg-primary text-white">حفظ</button>
                    </div>
                </div>
            );
        }
