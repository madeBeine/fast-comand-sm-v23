...
import { 
    ...
    PackageCheck, Zap, Activity, ShoppingCart, CheckCircle2, ChevronUp, ChevronDown, Loader2, FileWarning, X, Command, ArrowRight, Users, Store as StoreIcon, ScrollText, Scale, Calendar, PieChart as PieIcon, DollarSign, BellRing, Weight, BarChart2
} from 'lucide-react';
...
const WeightDetailsModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    orders: Order[];
    ...
    const stats = useMemo(() => {
        ...
        const monthlyData: { name: string; weight: number }[] = [];
        const monthMap: Record<string, number> = {};

        orders.forEach(o => {
            if (o.status === OrderStatus.CANCELLED || o.status === OrderStatus.NEW) return;
            
            const w = parseFloat(String(o.weight || 0));
            if (isNaN(w) || w <= 0) return;

            const d = new Date(o.orderDate);
            // Comparison based on timestamps to be accurate for today
            const dTime = d.getTime();

            // Daily: Compare Date Strings to ignore time component effectively
            if (d.toDateString() === now.toDateString()) daily += w;
            
            if (dTime >= startOfWeek.getTime()) weekly += w;
            if (dTime >= startOfMonth.getTime()) monthly += w;
            if (dTime >= startOfYear.getTime()) yearly += w;
        ...
        // Fill chart array
        Object.entries(monthMap).forEach(([name, weight]) => monthlyData.push({ name, weight: parseFloat(weight.toFixed(1)) }));

        return { 
            daily: parseFloat(daily.toFixed(1)), 
            weekly: parseFloat(weekly.toFixed(1)), 
    ...
    return (
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-[150] p-4 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-white dark:bg-gray-900 w-full max-w-lg rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b dark:border-gray-800 bg-gray-50 dark:bg-black/20 flex justify-between items-center">
                    <h3 className="text-xl font-black text-gray-900 dark:text-white flex items-center gap-2">
                        <Scale className="text-cyan-500"/> تفاصيل الأوزان
                    </h3>
                ...
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-cyan-50 dark:bg-cyan-900/20 p-4 rounded-2xl border border-cyan-100 dark:border-cyan-800">
                            <span className="text-xs font-bold text-cyan-600 dark:text-cyan-300 block mb-1">اليوم</span>
                            <span className="text-2xl font-black text-gray-800 dark:text-white">{stats.daily} <span className="text-sm text-gray-400">KG</span></span>
                        </div>
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-2xl border border-blue-100 dark:border-blue-800">
                            <span className="text-xs font-bold text-blue-600 dark:text-blue-300 block mb-1">آخر 7 أيام</span>
                            <span className="text-2xl font-black text-gray-800 dark:text-white">{stats.weekly} <span className="text-sm text-gray-400">KG</span></span>
                        </div>
                        <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-2xl border border-purple-100 dark:border-purple-800">
                            <span className="text-xs font-bold text-purple-600 dark:text-purple-300 block mb-1">هذا الشهر</span>
                            <span className="text-2xl font-black text-gray-800 dark:text-white">{stats.monthly} <span className="text-sm text-gray-400">KG</span></span>
                        </div>
                        <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-2xl border border-indigo-100 dark:border-indigo-800">
                            <span className="text-xs font-bold text-indigo-600 dark:text-indigo-300 block mb-1">هذه السنة</span>
                            <span className="text-2xl font-black text-gray-800 dark:text-white">{stats.yearly} <span className="text-sm text-gray-400">KG</span></span>
                        </div>
                    </div>

                    <div className="h-48 w-full bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 p-2">
                        <p className="text-xs font-bold text-gray-400 mb-2 px-2">الأوزان خلال الأشهر الماضية</p>
                        <ResponsiveContainer width="100%" height="90%">
                            <BarChart data={stats.monthlyData}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} opacity={0.2} />
                                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize: 10}} />
                                <Tooltip cursor={{fill: 'transparent'}} contentStyle={{borderRadius: '8px'}} />
                                <Bar dataKey="weight" fill="#06b6d4" radius={[4, 4, 0, 0]} barSize={30} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
...
const KPI_Card: React.FC<{ 
    ...
    isCurrency?: boolean;
    suffix?: string;
}> = ({ title, value, icon: Icon, color, subtitle, onClick, loading, isCurrency = true, suffix = 'MRU' }) => (
    <div 
        onClick={onClick}
        className={`min-w-[260px] md:min-w-0 flex-1 bg-white dark:bg-slate-800 p-5 rounded-[1.5rem] shadow-sm border border-slate-100 dark:border-slate-700/50 hover:shadow-md transition-all group relative overflow-hidden snap-start ${onClick ? 'cursor-pointer hover:scale-[1.02]' : ''}`}
    >
...
const StatGridItem: React.FC<{ label: string; value: number; icon: any; colorClass: string; onClick?: () => void }> = ({ label, value, icon: Icon, colorClass, onClick }) => (
    <div onClick={onClick} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border dark:border-gray-700 flex items-center justify-between shadow-sm active:scale-95 transition-transform cursor-pointer">
...
const Dashboard: React.FC<{
    ...
    return (
        <div className="space-y-6 pb-24 md:pb-10 max-w-full overflow-x-hidden">
            
            <WeightDetailsModal 
                isOpen={showWeightDetails}
                onClose={() => setShowWeightDetails(false)}
                orders={orders}
            />
            ...
            <div className="relative -mx-4 px-4 md:mx-0 md:px-0">
                <div className="flex md:grid md:grid-cols-2 lg:grid-cols-4 gap-4 overflow-x-auto pb-4 pt-1 snap-x snap-mandatory no-scrollbar">
                    {showFinancials ? (
                        <>
                            <KPI_Card title="صافي الأرباح" value={displayStats.profit} icon={TrendingUp} color="from-emerald-400 to-green-600" loading={isStatsLoading} subtitle="Net Profit" />
                            <KPI_Card 
                                title="إجمالي الأوزان" 
                                value={operationalStats.totalWeight.toFixed(1)} 
                                icon={Weight} 
                            ...
                            />
                            <KPI_Card title="الديون المستحقة" value={displayStats.debt} icon={AlertCircle} color="from-rose-400 to-red-600" loading={isStatsLoading} subtitle="Debt" />
                            <KPI_Card title="السيولة المستلمة" value={displayStats.cash} icon={Wallet} color="from-amber-400 to-orange-600" loading={isStatsLoading} subtitle="Cash Flow" />
                        </>
                    ) : (
                        <>
                            <KPI_Card title="الطلبات النشطة" value={operationalStats.activeOrders} icon={Package} color="from-blue-400 to-blue-600" loading={isStatsLoading} isCurrency={false} suffix="طلب" subtitle="Active Orders" />
                            <KPI_Card 
                                title="إجمالي الأوزان" 
                                value={operationalStats.totalWeight.toFixed(1)} 
                                icon={Weight} 
                                ...
                                onClick={() => setShowWeightDetails(true)} 
                            />
                            <KPI_Card title="جاهز للتسليم" value={operationalStats.readyForPickup} icon={Clock} color="from-orange-400 to-amber-600" loading={isStatsLoading} isCurrency={false} suffix="طلب" subtitle="Ready for Pickup" onClick={() => onFilterClick('ready_pickup')} />
                            <KPI_Card title="طلبات مكتملة" value={operationalStats.completedOrders} icon={CheckCircle2} color="from-teal-400 to-emerald-600" loading={isStatsLoading} isCurrency={false} suffix="طلب" subtitle="Completed Orders" onClick={() => onFilterClick(OrderStatus.COMPLETED)} />
                        </>
                    )}
                </div>
            </div>
            ...
            {(urgentStats.lateOrders > 0 || urgentStats.needsTracking > 0 || urgentStats.waitingWeight > 0 || urgentStats.pendingInvoice > 0 || urgentStats.pendingNotification > 0 || urgentStats.newOrders > 0) && (
                <div>
                    <h3 className="text-sm font-black text-slate-500 mb-3 px-1 flex items-center gap-2">
                        <AlertCircle size={16}/> إجراءات عاجلة (Action Center)
                    </h3>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <StatGridItem label="بانتظار الشحن" value={orderedCount} icon={ShoppingCart} colorClass="text-purple-600" onClick={() => onFilterClick(OrderStatus.ORDERED)}/>
                <StatGridItem label="شحنات بالطريق" value={displayStats.transitOrders} icon={Truck} colorClass="text-blue-600" onClick={() => onFilterClick(OrderStatus.SHIPPED_FROM_STORE)}/>
                <StatGridItem label="واصل / بالمخزن" value={displayStats.readyOrders} icon={Clock} colorClass="text-orange-600" onClick={() => onFilterClick('ready_pickup')}/>
                <StatGridItem label="طلبات مكتملة" value={completedOrdersCount} icon={CheckCircle2} colorClass="text-green-600" onClick={() => onFilterClick(OrderStatus.COMPLETED)}/>
            </div>
