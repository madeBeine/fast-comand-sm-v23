...
const SettingsPage: React.FC<SettingsPageProps> = ({ 
    ...
    const handleGenerateDemoData = async () => {
        ...
        try {
            // 1. Ensure Dependencies (Clients, Stores, Currencies)
            ...
            const defaultRate = currenciesData?.find(c => c.code === settings.defaultCurrency)?.rate || 10; // Fallback rate

            if (!currentClients?.length || !currentStores?.length) {
                throw new Error("فشل في إنشاء عملاء أو متاجر للربط");
            }

            // 2. Generate Orders based on Status Distribution
            const statuses = ['new', 'ordered', 'shipped_from_store', 'arrived_at_office', 'stored', 'completed'];
            const ordersToCreate: any[] = [];
            
            // Create 30 orders, cycling through statuses to ensure even distribution
            for (let i = 0; i < 30; i++) {
                ...
                // Financials
                const foreignPrice = Math.floor(Math.random() * 500) + 50; // 50 - 550
                const priceInMRU = Math.round(foreignPrice * defaultRate);
                const commission = Math.round(priceInMRU * 0.1); // 10%
                const shippingCost = ['arrived_at_office', 'stored', 'completed'].includes(status) ? 450 : 0; // Mock shipping cost
                const deliveryCost = 0; // Keeping simple
                
                const totalDue = priceInMRU + commission + shippingCost + deliveryCost;
                
                // Data Logic per Status
                let amountPaid = 0;
                let globalId = null;
                let tracking = null;
                let weight = null;
                ...
                if (status === 'new') {
                    // Randomize payment status for new orders (Unpaid, Partial, Paid)
                    const paymentScenario = i % 3; 
                    if (paymentScenario === 0) amountPaid = 0; // Unpaid
                    else if (paymentScenario === 1) amountPaid = Math.floor(totalDue / 2); // Partial
                    else amountPaid = totalDue; // Paid
                } 
                else {
                    // Logic for progressed statuses
                    if (status !== 'new') {
                    ...
                    if (['arrived_at_office', 'stored', 'completed'].includes(status)) {
                        weight = (Math.random() * 5 + 0.5).toFixed(1); // 0.5 - 5.5 KG
                        arrivalDate = new Date(Date.now() - 1000 * 60 * 60 * 24 * 2).toISOString(); // 2 days ago
                    }

                    if (['stored', 'completed'].includes(status)) {
                        location = `A${Math.floor(Math.random()*5)+1}-0${Math.floor(Math.random()*9)+1}`;
                        storageDate = new Date(Date.now() - 1000 * 60 * 60 * 24 * 1).toISOString(); // 1 day ago
                    }

                    if (status === 'completed') {
                        amountPaid = totalDue; // Must be fully paid
    ...
    const renderModalContent = () => {
        ...
        if (modalType === 'store') {
            return (
                <div className="space-y-5">
                    <h3 className="text-xl font-black border-b pb-3 dark:border-gray-700">بيانات المتجر</h3>
                    <div>
                    ...
                    <div>
                        <label className={labelClass}>مدة التوصيل التقديرية (أيام)</label>
                        <input type="number" value={editingItem.estimatedDeliveryDays || ''} onChange={e => setEditingItem({...editingItem, estimatedDeliveryDays: parseInt(e.target.value)})} className={inputClass} />
                    </div>
                    <div>
                        <label className={labelClass}>الشعار</label>
                        <label className="flex items-center gap-2 cursor-pointer bg-gray-50 dark:bg-gray-800 p-3 rounded-xl border border-dashed">
                            <Upload size={16}/>
                            <span>{editingItem.logo ? 'تغيير الشعار' : 'رفع شعار'}</span>
                        ...
                        {editingItem.logo && <img src={editingItem.logo} className="h-10 mt-2 object-contain"/>}
                    </div>
                    <div className="pt-4 flex justify-end gap-3">
                        <button onClick={closeModal} className="px-6 py-2 rounded-xl bg-gray-200 dark:bg-gray-700">إلغاء</button>
                        <button onClick={() => handleGenericSave('Stores', editingItem, setStores)} className="px-6 py-2 rounded-xl bg-primary text-white">حفظ</button>
                    </div>
                </div>
            );
        }
